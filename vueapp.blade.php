...
@endif
	<main>
		...
		<section id="login-view" class="login-shell" hidden>
			<form id="login-form" class="login-panel">
				<div id="auth-tabs" class="auth-tabs" role="tablist" aria-label="登录或注册">
				...
				<div id="auth-register-fields" hidden>
					<label>
						<span>用户名</span>
						<input id="register-username" name="register_username" autocomplete="username" minlength="3" maxlength="40" pattern="[A-Za-z0-9_\-\u4e00-\u9fff]+" required>
						<small>3～40 个字符，可使用中文、字母、数字、下划线和短横线。</small>
					</label>
					<label>
						<span>密码</span>
						<input id="register-password" name="register_password" type="password" autocomplete="new-password" minlength="8" required>
						<small>至少 8 个字符，请使用仅自己知道的密码。</small>
					</label>
					<label id="register-invite-field" hidden>
						<span>邀请码</span>
						<input id="register-invite-code" name="invite_code" autocomplete="one-time-code" maxlength="64" placeholder="XXXX-XXXX-XXXX">
						<small>邀请码仅可成功注册一次。</small>
					</label>
					<label>
						<span>确认密码</span>
		...
		<section id="profile-view" hidden>
			<div class="profile-page animate-up">
				...
				<div class="profile-actions">
					<button class="btn" type="button" data-action="open-my-activities">我的动态</button>
					<a id="profile-admin-link" class="btn" href="/gallery/myadmin/" hidden>管理员中心</a>
		...
		<section id="activities-view" hidden>
			<div class="activities-feed-header">
				<div>
					<button id="activities-scope-back" class="back-link" type="button" data-action="close-my-activities" aria-label="返回个人中心" hidden><span aria-hidden="true">&larr;</span></button>
			...
			<div class="compose-page-shell">
				...
				<section id="activity-composer" class="activity-composer compose-form-surface">
					<form id="activity-form">
						...
						<p class="compose-media-help">最多 20 个文件；图片不超过 25MB，视频不超过 250MB，总大小不超过 500MB。</p>
						<div id="activity-upload-progress" class="activity-upload-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0" hidden><span id="activity-upload-progress-bar"></span></div>
						<div class="activity-form-actions"><button class="btn" type="button" data-action="pick-activity-images">添加图片/视频</button><span id="activity-image-count" class="card-meta" aria-live="polite"></span><button id="activity-upload-cancel" class="btn" type="button" data-action="cancel-activity-upload" hidden>取消上传</button><button id="activity-submit" class="btn btn-primary" type="submit">发布</button></div>
					</form>
				</section>
			</div>
		</section>
	...
	<div id="activity-detail-modal" class="activity-detail-modal" role="dialog" aria-modal="true" aria-label="动态详情" aria-hidden="true" hidden>
		<article class="activity-detail-panel">
			...
			<div class="activity-detail-copy">
				...
				<div id="activity-detail-thumbs" class="activity-detail-thumbs" aria-label="动态媒体"></div>
	...
	<div id="activity-delete-dialog" class="activity-delete-dialog" role="dialog" aria-modal="true" aria-labelledby="activity-delete-title" aria-hidden="true" hidden>
		<div class="activity-delete-panel">
			<h2 id="activity-delete-title">删除这条动态？</h2>
			<p>动态、评论和全部媒体将被彻底删除，无法恢复。</p>
			<div class="activity-delete-actions"><button class="btn" type="button" data-action="cancel-delete-activity">取消</button><button id="activity-delete-confirm" class="btn btn-danger" type="button" data-action="confirm-delete-activity">彻底删除</button></div>
		</div>
	</div>
